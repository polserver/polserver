...
class Network:
  ...
  ## One step of the decoder, memoized: (node, byte) -> (bytes emitted, node to
  ## carry on from, whether the halt codeword ended the packet inside this byte).
  ## Every packet starts on a byte boundary and the halt codeword is followed by
  ## padding up to the next one, so the decoder never has to resume mid-byte and
  ## a whole byte can be walked in one lookup instead of eight.
  _STEP = {}

  ## Longest to wait for the server to accept. Loopback answers at once; without a
  ## bound, a shard that is not listening costs a full OS connect retry cycle, and
  ## this process is the first stage of cmake's pipeline and holds the whole run open.
  CONNECT_TIMEOUT = 10
  ...
  async def connect(cls, ip, port):
    ...
    '''
    # AF_INET, because the caller has already resolved the name to an IPv4 address
    # and the shard listens on IPv4. Left to itself getaddrinfo offers ::1 first on
    # a host that has it, and every connection would pay a refusal before its real try.
    reader, writer = await asyncio.wait_for(
        asyncio.open_connection(str(ip), port, family=socket.AF_INET),
        cls.CONNECT_TIMEOUT)
    sock = writer.get_extra_info('socket')
    if sock is not None:
      # The traffic is small requests answered by small replies, which is what
      # Nagle holds back waiting for an ack the other end is already delaying.
  ...
  def _plainLen(self):
    ...
    if cls is None:
      # Nothing here can be skipped over without knowing how long it is, and
      # guessing would desync the stream, so say so now rather than wait for
      # bytes that will never make a packet.
  ...
  def _nextCompressed(self):
    ...
    while pos < len(buf):
      ...
      pos += 1
      if halted:
        # The halt codeword is padded out to the end of its byte, so the next
        # packet starts at pos.
        del buf[:pos]
        self._node = 0
        self._pos = 0
        self._out = bytearray()
