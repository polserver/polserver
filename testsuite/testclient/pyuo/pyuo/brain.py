...
class Brain:
  ...
  def __init__(self, client, id = None):
    ...
    ## Set by whoever gives this brain something to do - the client thread when it
    ...
    while True:
      ...
      if self.timeout:
        ...
        while True:
          ...
          if remaining <= 0:
            break
          # Clear before the last look at both queues: a wakeup landing between
          # that look and the wait leaves the flag set, so the wait returns at
          # once rather than losing the signal.
          self.wakeup.clear()
  ...
  def processEvents(self):
    ...
    with self.eventsLock:
      if not self.events:
        return
      # Hand the whole queue over and leave an empty one behind, so the producer
      # is held up only for the swap rather than for a copy of what has piled up.
      events = self.events
      self.events = collections.deque()
    while events:
