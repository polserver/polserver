...
class Brain:
  ...
  def __init__(self, client, id = None):
    ...
    ## Set by whoever gives this brain something to do - the client task when it
    ## raises an event, the test harness when it queues a todo. The main loop waits
    ## on it instead of polling, so neither has to sit out a poll interval.
  ...
  async def run(self):
    ...
    if self.client.stopped.is_set():
      # The gate is opened by a completed login and by the client giving up, so
      # reaching it says only that the wait is over. A login does not always
      # complete - a character creation the server refuses drops the connection
      # instead - and waiting on one that never will holds up the whole run.
      #
      # The queue is deliberately left alone: the only thing in it is the crash the
      # client has already reported, and handing that to a brain whose onEvent turns
      # it into an exception would make a refusal that a test asked for look like a
      # failure.
    ...
    while True:
      ...
      # Wait for events. An order that arrived while we were idling should not
      # have to sit out the rest of the wait: this loop is what decides how long
      # a driven brain takes to react to anything asked of it, so it blocks on
      # the wakeup both producers set rather than on a poll interval.
      if self.timeout:
        ...
        while True:
          ...
          if remaining <= 0:
            break
          # Cleared here rather than after the wait, so a signal raised while the
          # queues above were being drained is not mistaken for a new one. Nothing
          # can arrive between the checks above and this wait - no await separates
          # them, so no producer gets to run - which is what makes that safe.
          self.wakeup.clear()
          try:
...
# The names the test scripts match on, keyed by event type. A table rather than
# the chain of comparisons this used to be, which was walked once per event. The
# eScript side lists the same strings as EVT_* constants in
# testsuite/pol/testpkgs/client/communication.inc - the two are edited together.
TYPESTR = {
  Event.EVT_INIT: "init",
