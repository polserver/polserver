...
class Mobile(UOBject):
  ...
  async def getEquipByLayer(self, layer):
    ...
    # Bounded: an unbounded wait parks the brain, which the shard cannot tell from a
    # dead client.
...
async def supervise(log, ctask, btask, grace=5.0):
  ...
  # The client first: when both failed, the client's failure is the cause.
...
class Client:
  ...
  PING_INTERVAL = 30
  ## Login handshake timeout. Nothing may wait unbounded: this process holds cmake's
  ## pipeline open.
  LOGIN_TIMEOUT = 30
  ...
  def __init__(self, id=None):
    ...
    ## Packets and todos in arrival order, in one queue so the loop waits on one thing.
    ...
    self.ping = 0
    ## Barrier pings awaiting an echo, oldest first, as (cookie, token). Cookie 0 is
    ## the keep-alive's and is never used here.
    self._syncPending = collections.deque()
    self._syncCookie = 0
    # Pulsed once the loop has handled a read. waitFor() blocks on it.
  ...
  async def _run(self):
    ...
    try:
      ...
      # Raised, not swallowed: a client that dies quietly leaves the shard waiting.
      raise
    finally:
      for t in (self._reader, self._pinger):
        if t is not None:
          t.cancel()
      # Release everyone waiting on this client before the socket goes.
      self.stopped.set()
  ...
  async def _readLoop(self):
    ...
    try:
      ...
      # Hand the loop the failure, so it stops waiting on a dead reader.
  ...
  async def _loop(self):
    ...
    while True:
      ...
      # Take everything queued at once: a burst handled one packet per turn would be
      # seen half applied.
      handled = todo = False
      while True:
  ...
  def handlePingPacket(self, pkt):
    ...
    if self._syncPending and self._syncPending[0][0] == pkt.seq:
      ...
      # The core answers 0x73 in order, so this cannot happen. Resynchronise and say so,
      # rather than wedge every later barrier.
      skipped = 0
      ...
      self.log.error("ping 0x%02x answered out of order, %d barrier(s) skipped",
                     pkt.seq, skipped)
    if token is None:
      # The keep-alive's ping, or a barrier its script gave up on. Harmless.
  ...
  def handleMovePacket(self, pkt):
    ...
    # Match the first move awaiting an ack.
    if not self.unmoves:
      raise RuntimeError(
    ...
    if not ack:
      ...
      self.moveid = -1
      # The core drops the requests in flight, so forget them too.
      self.unmoves.clear()
  ...
  def move(self, dir):
    ...
    # Numbered and sent with no await between, so moves cannot be reordered.
    self.moveid += 1
    if self.moveid > 0xff:
      self.moveid = 1
  ...
  async def waitFor(self, cond, timeout=None):
    ...
    while True:
      if self.stopped.is_set():
        # Nobody is left to make the condition true.
        return False
      now = time.monotonic()
  ...
  def processTodo(self, todo):
    ...
    if todo.type == brain.Event.EVT_EXIT:
      ...
      if hasattr(todo, 'parent') and todo.parent in self.objects:
        ...
        # No contents yet, or a parent that is no container: an empty listing.
  ...
  async def receive(self, expect=None, timeout=None):
    ...
    '''
    # One reader per connection: the framers in net.py keep state across reads.
