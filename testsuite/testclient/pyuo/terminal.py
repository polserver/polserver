...
## How often a modal dialog looks for a keystroke. Imperceptible to a person,
## and it keeps every curses call on the one task curses may be called from.
KEY_POLL = 0.02
...
class InputDialog(BaseDialog):
  ...
  async def edit(self):
    ...
    # Textbox.edit() owns a blocking getch() loop of its own, which would hold up
    # every other task on the loop for as long as the user was typing. This is the
    # same loop with the read made non-blocking: do_command() is what edit() calls
    # for each key, and it returns false once Ctrl-G is struck.
    ewin.nodelay(True)
    curses.curs_set(2)
    try:
      while True:
...
class SelectDialog(BaseDialog):
  ...
  async def select(self):
    ...
    curses.flushinp()
    # Non-blocking, so waiting on the user does not stop the client's task reading
    # its socket - curses is not safe to call from anywhere but this one task, so
    # the wait has to happen here rather than on a thread.
