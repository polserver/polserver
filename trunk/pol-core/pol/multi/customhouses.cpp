...
#define BYTES_PER_TILE 5
	...
	void CustomHouseStopEditing( Mobile::Character* chr, UHouse* house )
	{
	  ...
	  move_character_to( chr, house->x + def.minrx, house->y + def.maxry + 1, house->z, Core::MOVEITEM_FORCELOCATION, NULL );
    ...
    void CustomHousesCommit( Core::PKTBI_D7* msg )
	{
	  ...
	  if ( sd.config_nodie( "misc/customhousecommit.ecl", 0, 0 ) )
	  {
		if ( sd.exists() )
		{
		  ...
		  if ( Core::start_script( sd, make_mobileref( chr ), new Module::EMultiRefObjImp( house ), house->WorkingDesign.list_parts() ) != NULL )
			return;
		}
	  }
