...
namespace Pol {
  ...
  namespace Module {
	...
	Bscript::BObjectImp* SQLExecutorModule::mf_ConnectToDB()
	{
      ...
      Core::BSQLConnection *sql = new Core::BSQLConnection( );
	  if ( sql->getLastErrNo() )
	  {
	...
	Bscript::BObjectImp* SQLExecutorModule::mf_SelectDb()
	{
	  ...
	  const String *db;
      if ( !( sql = EXPLICIT_CAST( Core::BSQLConnection*, Bscript::BObjectImp* )( getParamImp( 0, Bscript::BObjectImp::OTSQLConnection ) ) ) || !( db = getStringParam( 1 ) ) )
	  {
		return new BError( "Invalid parameters" );
	  }
	  if ( !sql->select_db( *db ) )
	...
	Bscript::BObjectImp* SQLExecutorModule::mf_Query()
	{
      Core::BSQLConnection *sql;
	  const String *query;
      if ( !( sql = EXPLICIT_CAST( Core::BSQLConnection*, Bscript::BObjectImp* )( getParamImp( 0, Bscript::BObjectImp::OTSQLConnection ) ) ) || !( query = getStringParam( 1 ) ) )
	  {
		return new BError( "Invalid parameters" );
	  }
	...
	Bscript::BObjectImp* SQLExecutorModule::mf_NumFields()
	{
      ...
      if ( !( result = EXPLICIT_CAST( Core::BSQLResultSet*, Bscript::BObjectImp* )( getParamImp( 0, Bscript::BObjectImp::OTSQLResultSet ) ) ) )
	  {
		return new BError( "Invalid parameters" );
	  }
	...
	Bscript::BObjectImp* SQLExecutorModule::mf_FieldName()
	{
      ...
      if ( !( result = EXPLICIT_CAST( Core::BSQLResultSet*, Bscript::BObjectImp* )( getParamImp( 0, Bscript::BObjectImp::OTSQLResultSet ) ) ) || !getParam( 1, index ) )
	  {
		return new BError( "Invalid parameters" );
	  }
	...
	{
      Core::BSQLConnection *sql;
      if ( !( sql = EXPLICIT_CAST( Core::BSQLConnection*, Bscript::BObjectImp* )( getParamImp( 0, Bscript::BObjectImp::OTSQLConnection ) ) ) )
		return new BError( "Invalid parameters" );
	  sql->close();
	  return new BLong( 1 );
	}
	Bscript::BObjectImp* SQLExecutorModule::mf_FreeResult()
	{
      Core::BSQLResultSet *result;
      if ( !( result = EXPLICIT_CAST( Core::BSQLResultSet*, Bscript::BObjectImp* )( getParamImp( 0, Bscript::BObjectImp::OTSQLResultSet ) ) ) )
	  {
