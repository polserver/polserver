...
class Change:
    ...
    def __init__(self, date, author):
        if date is None:
            ...
            if len(date) == 5:
                #use a fixed year to have defined behaviour, will not be written later
                self.date = datetime.datetime.strptime(date+'-1900', '%m-%d-%Y').date()
            elif len(date) == 8:
            ...
            for change in rel:
                ...
                for sub in change:
                    ...
                    if sub.lines:
                        ...
                        # older python version escape quotes, keep it that way
                        chel.text = chel.text.replace('"','~~~QUOTE~~~')
