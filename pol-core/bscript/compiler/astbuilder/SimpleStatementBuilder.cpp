...
void SimpleStatementBuilder::add_var_statements( EscriptParser::VarStatementContext* ctx,
...
void SimpleStatementBuilder::add_var_statements( EscriptParser::UnambiguousVarStatementContext* ctx,
                                           std::vector<std::unique_ptr<Statement>>& statements )
{
  if ( auto variable_declaration_list = ctx->unambiguousVariableDeclarationList() )
  {
    for ( auto decl : variable_declaration_list->unambiguousVariableDeclaration() )
    {
      auto loc = location_for( *decl );
      std::string name = text( decl->IDENTIFIER() );
      std::unique_ptr<VarStatement> var_ast;

      if ( auto initializer_context = decl->unambiguousVariableDeclarationInitializer() )
      {
        if ( initializer_context->ARRAY() )
        {
          var_ast = std::make_unique<VarStatement>( loc, std::move( name ), true );
...
std::unique_ptr<ConstDeclaration> SimpleStatementBuilder::const_declaration(
    EscriptParser::UnambiguousConstStatementContext* ctx )
{
  auto variable_declaration = ctx->unambiguousVariableDeclaration();
  auto identifier = text( variable_declaration->IDENTIFIER() );
  auto expression_context =
      variable_declaration->unambiguousVariableDeclarationInitializer()->unambiguousExpression();
  auto value = expression( expression_context );
...
std::unique_ptr<EnumDeclaration> SimpleStatementBuilder::enum_declaration(
    EscriptParser::UnambiguousEnumStatementContext* ctx )
{
  std::vector<std::string> names;
  std::vector<std::unique_ptr<Expression>> expressions;
  if ( auto enum_list = ctx->unambiguousEnumList() )
  {
    std::string last_identifier;
    for ( auto entry : enum_list->unambiguousEnumListEntry() )
    {
      auto source_location = location_for( *entry );
      std::string identifier = text( entry->IDENTIFIER() );
      std::unique_ptr<Expression> value;
      if ( auto expr_ctx = entry->unambiguousExpression() )
      {
        value = expression( expr_ctx );
      }
      else if ( !last_identifier.empty() )
  ...
  auto source_location = location_for( *ctx );
  std::string identifier = text( ctx->IDENTIFIER() );
  return std::make_unique<EnumDeclaration>( source_location, std::move( identifier ), names,
...
std::unique_ptr<Expression> SimpleStatementBuilder::variable_initializer(
    EscriptParser::UnambiguousVariableDeclarationInitializerContext* ctx )
{
  if ( auto expr = ctx->unambiguousExpression() )
    return expression( expr );
  else
    return std::unique_ptr<Expression>( new StringValue( location_for( *ctx ), "" ) );
}
...
std::unique_ptr<ReturnStatement> SimpleStatementBuilder::return_statement(
    EscriptParser::UnambiguousReturnStatementContext* ctx )
{
  auto source_location = location_for( *ctx );

  std::unique_ptr<Expression> result;
  if ( auto expression_ctx = ctx->unambiguousExpression() )
    result = expression( expression_ctx );
  else
    result = std::make_unique<StringValue>( source_location, "" );
