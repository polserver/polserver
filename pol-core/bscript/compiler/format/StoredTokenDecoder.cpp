...
void StoredTokenDecoder::decode_to( const StoredToken& tkn, fmt::Writer& w )
{
  ...
  case CTRL_LEAVE_BLOCK:
    ...
    break;  // offset is number of variables to remove
  ...
  case TOK_FUNC:
  {
    ...
    {
      w << "module index " << module_id
        << " exceeds module_descriptors size " << module_descriptors.size();
      break;
    }
    auto& module = module_descriptors.at( module_id );
    if ( function_index >= module.functions.size() )
    {
      w << "function index " << function_index
          << " exceeds module.functions size " << module.functions.size();
      break;
    }
  ...
  case INS_SUBSCRIPT_ASSIGN_CONSUME:
    w << "subscript assign (" << tkn.offset << " indexes) and consume";
    break;
  case INS_MULTISUBSCRIPT:
    w << "multi subscript assign (" << tkn.offset << " indexes)";
    break;
  case INS_MULTISUBSCRIPT_ASSIGN:
    w << "multi subscript assign (" << tkn.offset << " indexes) and consume";
    break;
...
void StoredTokenDecoder::decode_casejmp_table( fmt::Writer& w, unsigned offset ) const
{
  ...
  for ( ;; )
  {
    ...
    offset += 2;
    ESCRIPT_CASE_TYPES type = static_cast<ESCRIPT_CASE_TYPES>( data.at( offset ) );
    offset++;
