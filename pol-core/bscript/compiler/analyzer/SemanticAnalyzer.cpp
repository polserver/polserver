...
void SemanticAnalyzer::visit_basic_for_loop( BasicForLoop& node )
{
  ...
  LocalVariableScope scope( scopes, node.debug_variables );
  scope.create( node.identifier, false, false, node.source_location );
  scope.create( "_" + node.identifier + "_end", false, false, node.source_location );
...
{
  LocalVariableScope scope( scopes, block.debug_variables );
...
{
  // INFO_PRINT << "sem an break: label = " << node.label << "\n";
...
void SemanticAnalyzer::visit_foreach_loop( ForeachLoop& node )
{
  ...
  LocalVariableScope scope( scopes, node.debug_variables );
  scope.create( node.iterator_name, false, false, node.source_location );
  scope.create( "_" + node.iterator_name + "_expr", false, false, node.source_location );
  scope.create( "_" + node.iterator_name + "_iter", false, false, node.source_location );
...
void SemanticAnalyzer::visit_function_parameter_declaration( FunctionParameterDeclaration& node )
{
  ...
  bool warn_if_used = node.unused;
  bool warn_if_unused = !node.unused;
  scopes.current_local_scope()->create( node.name, warn_if_used, warn_if_unused,
...
{
  // node.source_location.info( "sem an" );
  // INFO_PRINT << "sem an identifier: " << node.describe() << "\n";
...
void SemanticAnalyzer::visit_module_function_declaration( ModuleFunctionDeclaration& )
{
  // INFO_PRINT << "SemanticAnalyzer::visit_module_function_declaration: " + node.describe() +
  // "\n";

  // don't generate locals for children
  // visit_children( node );
}

void SemanticAnalyzer::visit_program( Program& program )
{
  LocalVariableScope scope( scopes, program.debug_variables );
  for ( auto& child : program.parameter_list().children )
  {
    child->accept( *this );
  }

  visit_children( program.body() );

  program.locals_in_block = scope.get_block_locals();
}

void SemanticAnalyzer::visit_program_parameter_list( ProgramParameterList& node )
{
  for ( auto& child : boost::adaptors::reverse( node.children ) )
  {
    child->accept( *this );
...
{
  // INFO_PRINT << "sem an var " + node.name + "\n";
  if ( constants.find( node.name ) )
  {
    report.error( node, node.name, " is already a defined constant.\n" );
    return;
  }

  if ( auto local_scope = scopes.current_local_scope() )
  {
    node.variable = local_scope->create( node.name, false, false, node.source_location );
  }
  else
  {
