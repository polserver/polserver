...
public:
  ...
  void array_declare();
  void assign();
  void assign_subscript_consume( unsigned indexes );
  void assign_suscript( unsigned indexes );
  ...
  void function_reference( unsigned parameter_count, FlowControlLabel& label );
  ...
  void set_member_by_operator( BTokenId token_id, MemberID member_id );
  void struct_create();
  ...
  void unary_operator( BTokenId token_id );
  void uninit();
  void value( double );
  void value( int );
...
private:
  ...
  void register_with_label( FlowControlLabel& label, unsigned offset );
