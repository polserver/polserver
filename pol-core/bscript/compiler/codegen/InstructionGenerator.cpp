...
void InstructionGenerator::visit_array_element_access( ArrayElementAccess& acc )
{
...
void InstructionGenerator::visit_assign_subscript( AssignSubscript& node )
{
  visit_children( node );
  update_debug_location( node );
  ...
  if ( node.consume )
  {
    emit.assign_subscript_consume( num_indexes );
  }
  else
  {
    if ( num_indexes == 1 )
      emit.assign_suscript( num_indexes );
    else
...
void InstructionGenerator::visit_case_statement( CaseStatement& node )
{
  ...
  auto& holder = node.dispatch_groups_holder();
  for ( int i = 0, c = holder.children.size(); i < c; ++i )
  {
    CaseDispatchGroup& group = holder.dispatch_group( i );
...
{
  update_debug_location( node );
  // INFO_PRINT << "** visit identifier " << node.name << "\n";
...
void InstructionGenerator::visit_if_then_else_statement( IfThenElseStatement& node )
{
  ...
  //           << "  - predicate: " << node.predicate()->to_string_tree() << "\n"
  //           << "  - block: " << node.block()->to_string_tree() << "\n";
...
void InstructionGenerator::visit_module_function_declaration( ModuleFunctionDeclaration& )
{
  // nothing to do (in particular, don't generate code for parameters)
}
...
void InstructionGenerator::visit_user_function( UserFunction& user_function )
{
  ...
  // INFO_PRINT << "Code Generator visit-user_function " << node.name << "\n";
  FlowControlLabel& label = user_function_labels[user_function.name];
  emit.label( label );
  visit_children( user_function );

  // INFO_PRINT << " last_statement" << ( last_statement ? last_statement->describe() : "n/a" )
  //           << "\n";
