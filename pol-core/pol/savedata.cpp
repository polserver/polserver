...
/// items.txt, formatted on every thread but written in zone order.
///
/// It is the one save file whose block order is still readable afterwards: on load these go back
/// into the zone vectors in file order, and that is the order every WorldIterator walk hands to
/// scripts. Hence `ordered` - its pieces are held until the last one is formatted and then
/// written in one pass. A large shard's items.txt is about a tenth of its storage.txt, so holding
/// it costs little, and a thread of its own would have cost far more: measured single-threaded
/// against fifteen other threads formatting storage, it was the whole critical section.
...
/// One mobile per piece, its worn items included: a mobile and its equipment are one indivisible
/// unit, and nothing ties one mobile's blocks to another's. Equipment finds its owner by serial
/// and pcs.txt is read before pcequip.txt, so the two files do not constrain each other either.
  ...
  SaveContext::finished = std::async(
       ...
       callback = std::move( callback )]() mutable
      {
        ...
        try
        {
          ...
          // pool of work. Giving any of them a thread of its own makes the save wait for that one
          // file. Run on this thread rather than as a pool task: write_parallel waits on pool work
          // at the end, and a pool thread waiting on the pool can leave nothing free to finish.
          try
          {
