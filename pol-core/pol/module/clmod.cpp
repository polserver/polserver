...
BObjectImp* ClilocExecutorModule::mf_SendSysMessageCL()
{
  ...
  if ( getCharacterParam( exec, 0, chr ) && getParam( 1, cliloc_num ) &&
       getStringParam( 2, oText ) && getParam( 3, font ) && getParam( 4, color ) )
  {
    ...
    Core::send_sysmessage_cl( chr->client, cliloc_num, oText->value(), font, color );
    return new BLong( 1 );
  }
  else
  {
...
BObjectImp* ClilocExecutorModule::mf_PrintTextAboveCL()
{
       ...
       getStringParam( 2, oText ) && getParam( 3, font ) && getParam( 4, color ) )
  {
    ...
    say_above_cl( obj, cliloc_num, oText->value(), font, color );
    return new BLong( 1 );
  }
  else
  {
...
BObjectImp* ClilocExecutorModule::mf_PrintTextAbovePrivateCL()
{
  ...
  if ( getCharacterParam( exec, 0, chr ) && getUObjectParam( exec, 1, obj ) &&
       getParam( 2, cliloc_num ) && getStringParam( 3, oText ) && getParam( 4, font ) &&
       getParam( 5, color ) )
  {
    ...
    private_say_above_cl( chr, obj, cliloc_num, oText->value(), font, color );
    return new BLong( 1 );
  }
  else
  {
