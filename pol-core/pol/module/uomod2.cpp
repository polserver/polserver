    ...
    /* chr_or_array, layout, txtlines, x=0, y=0, flags=0,gumpid=0*/ )
{
  ...
  ObjArray* layout_arr;
  ObjArray* data_arr;
  if ( !( chr_or_array && exec.getObjArrayParam( 1, layout_arr ) &&
          exec.getObjArrayParam( 2, data_arr ) && exec.getParam( 3, x ) && exec.getParam( 4, y ) &&
          exec.getParam( 5, flags ) && exec.getParam( 6, gump_id ) ) )
  {
    return new BError( "Invalid parameter" );
  ...
  auto send = [this]( auto& msg, auto chr, auto gumpid )
  {
    auto len = msg->offset;
    msg->offset = 3;
    msg->template Write<u32>( chr->serial_ext );
  ...
  PktHelper::PacketOut<PktOut_DD> msg;
  PktHelper::PacketOut<PktOut_B0> msg_unc;
  for ( auto* chr : characters )
  {
    ...
    else
    {
      if ( msg_unc->offset == 1 )
      {
        if ( auto error = buildUnCompressedGumpMenu( msg_unc, layout_arr, data_arr, x, y, gumpid );
             !error.empty() )
          return new BError( error );
      }
      send( msg_unc, chr, gumpid );
    }
    send_chrs->addElement( chr->make_ref() );
  }
  return send_chrs.release();
}
