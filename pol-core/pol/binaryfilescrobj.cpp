...
namespace Pol {
  namespace Core {
	...
	BBinaryfile::BBinaryfile( string filename, unsigned short mode, bool _bigendian ) : Bscript::BObjectImp( OTBinaryFile ),
	  _filename( filename ),
	...
	fstream::pos_type BinFile::FileSize( Bscript::Executor& exec )
	{
	  if ( !_file.is_open() )
