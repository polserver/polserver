...
void start_client_char( Network::Client* client )
{
  ...
  client->chr->lastxyz = Pos3d( 0, 0, 0 );

  client->gd->music_region = gamestate.musicdef->getregion( Pos2d( 0, 0 ), client->chr->realm() );
  client->gd->justice_region =
      gamestate.justicedef->getregion( Pos2d( 0, 0 ), client->chr->realm() );
  client->gd->weather_region =
      gamestate.weatherdef->getregion( Pos2d( 0, 0 ), client->chr->realm() );
...
void char_select( Network::Client* client, PKTIN_5D* msg )
{
  ...
  if ( chosen_char->lastxyz == Pos3d( 0, 0, 0 ) )
  {
    chosen_char->lastxyz = chosen_char->pos().xyz();
  }
