...
function (compareresult scriptname result)
  ...
  if (${outlen} EQUAL ${tstlen})
    ...
    message("\"${tst}\"")
    message(SEND_ERROR "${scriptname} Testdata length differs expected=${outlen} actual=${tstlen}")
    cleanup(${scriptname})
  endif()
endfunction()
...
# start of test
function (testwithcompiler errfilesuffix antlroption)
  ...
  foreach(script ${scripts})
    ...
    if ("${subtest}" STREQUAL "ternary")
      if ("${errfilesuffix}" STREQUAL "1")
        message("Skipping ${script} because OG compiler does not have a ternary operator.")
        continue()
      endif()
    endif()
    string(REPLACE ".src" "" scriptname "${script}")
    if (EXISTS "${testdir}/${scriptname}.out" OR EXISTS "${testdir}/${scriptname}.err${errfilesuffix}")
      execute_process( COMMAND ${ecompile} ${antlroption} -q -C ecompile.cfg ${script}
        RESULT_VARIABLE ecompile_res
        OUTPUT_VARIABLE ecompile_out
        ERROR_VARIABLE ecompile_out)
      if (EXISTS "${testdir}/${scriptname}.err${errfilesuffix}")
        if("${ecompile_res}" STREQUAL "0")
          cleanup(${scriptname})
          message(SEND_ERROR "${scriptname}.src compiled, but should not")
        else()
          checkprocfailure("${ecompile_out}" "${testdir}/${scriptname}.err${errfilesuffix}" success)
          if (NOT ${success})
            cleanup(${scriptname})
...
testwithcompiler("1" "-g-")
testwithcompiler("2" "-g")
